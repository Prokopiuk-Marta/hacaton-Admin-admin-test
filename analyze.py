...
- "neutral": проблему вирішено, але після довгих чи плутаних пояснень. АБО проблему не вирішено, але 
оператор професійно все пояснив і клієнт спокійно це прийняв
- "unsatisfied": проблема клієнта не була вирішена, але окрім цього оператор міг ввести себе нетактовно, 
грубіянити або ж відмовився допомогти.

2. Оцінка якості роботи оператор від 1 до 5:
- 5: швидке розуміння суті проблеми, чіткий алгоритм вирішення проблеми, ідеальна робота.
- 4: проблема вирішена, але оператор ставив зайві запитання, але при цьому клієнт задоволений, не 
вирішили проблему, оскільки це не стосується даної компанії
- 3: оператор був ввічливим, але рішення не знайшов; або знайшов, але дуже довго клієнт чекав; або не 
врахував стан клієнта наприклад, клієнт боїться, а оператор просто кидає сухі інструкції.
- 2: оператор дає шаблонні відповіді, які не стосуються конкретної проблеми клієнта, ігнорує частину запитань.
...
- "ignored_question": клієнт запитав про одне (наприклад, "а що з роутером?"), а оператор відповів 
про інше або проігнорував.
- "incorrect_info": оператор дає абсурдні або технічно неграмотні поради.
- "rude_tone": зверхність, сарказм, відсутність базового привітання/прощання, знецінення проблеми клієнта.
- "no_resolution": діалог завершився, але клієнту не сказали, що робити далі (навіть якщо це заявка на 
майстра).
- "unnecessary_escalation": оператор міг допомогти сам, але відправив клієнта до іншого відділу чи 
